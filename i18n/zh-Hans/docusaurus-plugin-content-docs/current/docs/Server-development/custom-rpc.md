...
## 处理程序运行在游戏线程上 {#handlers-run-on-the-game-thread}

`On...` 方法会被调度到服务器的更新线程上，也就是你的系统执行所在的线程。你不需要做任何跨线程处理：可以直接通过 `EcsApi` 读写组件，而且处理程序永远不会看到处于中间状态的实体，因为它不会在系统运行期间执行。

这一点在 `0.4.0` 中发生了变化。处理程序以前是在解析数据包时于网络线程上调用的，这意味着处理程序和系统可能同时访问实体存储。如果你曾把处理程序的函数体包在 `RunOnGameThread` 里，现在可以去掉了，调度已经由 SDK 完成。

耗时较长的工作在这里仍然是错误的做法，只是原因和以前不同了：处理程序一慢就会拖慢所有人的 tick。如果响应依赖只有系统才能得出的状态，就把请求保存下来，让系统去处理，做法见[游戏系统](systems#systems-and-rpc-handlers-together)。
