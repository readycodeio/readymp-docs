...
OblivionMP and WukongMP share the relay server and the core SDK, so everything that shipped in [WukongMP `0.4.0`](/releases/wukongmp-sdk-040) ships here too, against Oblivion's own components.
...
Three new additions here, all local to the client that calls them: `SDK.Chat` writes to the in-game chat log, `SDK.GameMessage` drives the game's own on-screen widgets, and `SDK.Markers` puts floating labels over other players for nameplates and callouts. See [Messages and markers](/oblivion-mp/docs/Client/messages-and-markers).
...
The `0.1.1` documentation stays available in the version dropdown. If you find something breaking, come find us on the [OblivionMP Discord](https://discord.gg/obmp).
